...
years = [x for x in range(2006, 2007)]
print(months)
print(years)
# --------- End Testing ---------
...
def get_meter_data(meter_id_in, meter_data_in, mydb_connection):
    ...
    connect.insert_to_database(
        data_in_month, mydb_connection, TABLE_NAME_DATA_MONTH)
    print(data_in_month)  # testing


if __name__ == "__main__":
    start_time = time.time()  # testing
    logging.basicConfig(filename="Error_at_resample_log.log", filemode='w',
                        format='%(asctime)s - %(levelname)s - %(message)s')
    # sys.exit()# testing
    mydb_sqlalchemy = connect.database_connection_with_sqlalchemy()
    sql_query_get_meter_id = "SELECT MeterId FROM Meter"
    MeterID = connect.read_from_database(
        sql_query_get_meter_id, mydb_sqlalchemy)
    meter_id_to_array = MeterID.values.reshape(-1,).tolist()
    print(meter_id_to_array)
    for meter_id in meter_id_to_array:
        ...
        try:
            ...
            sql_query_get_meter_data = "SELECT MAX(KWH) AS KWH, DATE_FORMAT(ReadAt, \"%Y-%m-%d 00:00:00\") AS ReadAt FROM MeterData WHERE MeterId={0} AND YEAR(ReadAt)={1} AND MONTH(ReadAt)={2} GROUP BY YEAR(ReadAt), MONTH(ReadAt), DAY(ReadAt)".format(
                meter_id, YEAR, MONTH)  # Real Use
            meter_data = connect.read_from_database(
                sql_query_get_meter_data, mydb_sqlalchemy)
            print(meter_data)
            ...
            logging.error("Type {0} occurred. Arguments:\n{1!r}".format(
                type(err).__name__, err.args))  # testing
            continue  # testing
